...
public protocol WLISideMenuDelegate: class {
...
public protocol WLISideMenuProtocol: class {
    var sideMenu : WLISideMenu? { get }
    func setContentViewController(_ contentViewController: UIViewController)
}

public enum WLISideMenuAnimation : Int {
    case none
    case `default`
}
/**
...
public extension UIViewController {
    ...
    */
    ...
    func fixSideMenuSize() {
        if let navController = navigationController as? WLISideMenuNavigationController {
    ...
    */
    func sideMenuController () -> WLISideMenuProtocol? {
        var iteration : UIViewController? = parent
        if (iteration == nil) {
            return topMostController()
        }
        repeat {
            if (iteration is WLISideMenuProtocol) {
                return iteration as? WLISideMenuProtocol
    ...
    internal func topMostController () -> WLISideMenuProtocol? {
        var topController : UIViewController? = UIApplication.shared.keyWindow?.rootViewController
        
        if (topController is UITabBarController) {
            topController = (topController as! UITabBarController).selectedViewController
        }
        var lastMenuProtocol : WLISideMenuProtocol?
        while (topController?.presentedViewController != nil) {
            if(topController?.presentedViewController is WLISideMenuProtocol) {
                lastMenuProtocol = topController?.presentedViewController as? WLISideMenuProtocol
            }
        ...
        if (lastMenuProtocol != nil) {
            ...
            return topController as? WLISideMenuProtocol
        }
    }
}

open class WLISideMenu : NSObject, UIGestureRecognizerDelegate {
    ...
    open weak var delegate : WLISideMenuDelegate?
    ...
    */
    public init(sourceView: UIView, menuPosition: ENSideMenuPosition, blurStyle: UIBlurEffect.Style = .light) {
        ...
        self.panRecognizer = UIPanGestureRecognizer(target: self, action: #selector(WLISideMenu.handlePan(_:)))
        panRecognizer!.delegate = self
        sourceView.addGestureRecognizer(panRecognizer!)

        // Add right swipe gesture recognizer
        let rightSwipeGestureRecognizer = UISwipeGestureRecognizer(target: self, action: #selector(WLISideMenu.handleGesture(_:)))
        rightSwipeGestureRecognizer.delegate = self
        rightSwipeGestureRecognizer.direction =  .right

        // Add left swipe gesture recognizer
        let leftSwipeGestureRecognizer = UISwipeGestureRecognizer(target: self, action: #selector(WLISideMenu.handleGesture(_:)))
...
extension WLISideMenu: UIDynamicAnimatorDelegate {
